...
| `NEXT_PUBLIC_SENTRY_DSN` | Browser + server | Sentry error tracking + session replay + logs. Safe to expose (send-only). Unset = no error reporting |
| `SENTRY_AUTH_TOKEN` | Build only (optional) | Uploads source maps for readable stack traces. Org/project (`fluencers-group` / `javascript-nextjs`) are set in `next.config.ts` |
