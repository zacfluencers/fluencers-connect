...
- **Campaign brief** (inside the deal room) — booking stays **one-click** (pay → booking created), then the brand fills in a **brief** here. Grouped sections: **Campaign details** (name, objective, target audience), **Content** (platform, deliverables, creative brief, talking points, call-to-action), **Requirements** (must include, avoid, deadline), **Commercial** (payment notes, usage rights), a **Product** choice, and **reference file uploads**. The **Product** choice is a toggle: *No product*, *I'll ship it* (reveals a tracking-number field), or *Creator orders it* (reveals a product link + a 100%-off discount code). Only the **brand** can edit; the **creator** sees a tidy read-only version and gets a notification when the first brief is added. A brand with no brief yet lands straight in edit mode (the post-booking nudge). The brief can be edited any time. Files go to a public **`briefs`** storage bucket (100MB each).
