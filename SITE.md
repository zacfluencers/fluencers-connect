...
## Testing
Automated browser tests live in the `e2e/` folder and use **Playwright**.
- **Run them:** `npm run test:e2e` (or `npm run test:e2e:ui` for the visual runner). They test against your running preview at `http://localhost:3000`.
- **What's covered today:** homepage / marketplace / login / signup render, the 404 page, sign-up flow, and a failed login. These always run.
- **The booking flow** (`e2e/booking.spec.ts`) logs in as a brand and books a creator. It only runs if you provide a test brand login via environment variables (`TEST_BRAND_EMAIL`, `TEST_BRAND_PASSWORD`); otherwise it skips itself, so the suite is always green. Example: `TEST_BRAND_EMAIL=you@example.com TEST_BRAND_PASSWORD=… npm run test:e2e`.
