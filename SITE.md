...
- 2026-06-19: **Real content delivery + two-way flow.** The deal room's "Content delivery" panel was a mockup — it now has **working uploads**: the creator uploads deliverables (video/images, 150MB each, with progress), the brand previews and downloads them. Added **backward steps** so the booking can move both ways between stages (creator can step back / withdraw a submission); Approve and Decline stay final because they move money. New `booking_deliverables` table + `deliverables` storage bucket (migration 0018).
